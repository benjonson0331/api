...
def get_trackingNo(cur):
	sql = '''IF OBJECT_ID('TEMPDB..#winit_ebay') IS NOT NULL BEGIN DROP TABLE #winit_ebay END
			 ...
			 WHERE 
			 	...
			 	(A.最终使用服务方式 LIKE '%USPS%' OR 
			 	 A.最终使用服务方式 LIKE '%UPS%' OR 
			 	 A.最终使用服务方式 LIKE '%XDP%' OR 
			 	 A.最终使用服务方式 LIKE '%TOLL%' OR 
			 	 A.最终使用服务方式 = 'no postageservice' OR 
			 	 A.最终使用服务方式 LIKE '%AU Post%' OR
			 	 A.最终使用服务方式 LIKE '%DPD%' OR
			 	 A.最终使用服务方式 LIKE '%P2P%')
			 ...
			 where
			 	...
			 	(a.PostageService LIKE '%USPS%' OR 
			 	 a.PostageService LIKE '%UPS%' OR 
			 	 a.PostageService LIKE '%XDP%' OR 
			 	 a.PostageService LIKE '%TOLL%' OR 
			 	 a.PostageService = 'no postageservice' OR 
			 	 a.PostageService LIKE '%AU Post%' OR 
			 	 a.PostageService LIKE '%DPD%' OR
			 	 a.PostageService LIKE '%P2P%')
			 select *
			 from #winit_ebay
			 union
			 SELECT *
